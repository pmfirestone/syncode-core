...
pub(crate) struct LRTables {
    /// The grammar we are building tables for.
    grammar: Grammar,
    /// The item sets for this grammar.
    item_sets: Vec<HashSet<Item>>,
    /// The goto sets for this grammar, indexed by symbol.
    goto_sets: HashSet<(String, HashSet<Item>)>,
}
...
impl LRTables {
        ...
        if self.is_terminal(symbol) {
            ...
            for production in self.grammar.productions.iter().filter(|p| *p.lhs == *symbol) {
                ...
                for inner_symbol in production.rhs.iter() {
                    ...
                    let first = self.symbol_first(inner_symbol);
    ...
    fn string_first(&self, string: Vec<String>) -> HashSet<String> {
        ...
        for (idx, outer_symbol) in string.into_iter().enumerate() {
            let first_of_this_symbol = self.symbol_first(&outer_symbol);
    ...
    fn closure(&self, mut items: HashSet<Item>) -> HashSet<Item> {
        ...
        'repeat: loop {
            ...
            for item in items.iter() {
                ...
                for production in &self.grammar.productions {
                    ...
                    for terminal in self.string_first(little_item_set) {
                        new_items.push(Item {
    ...
    fn goto(&self, items: &HashSet<Item>, symbol: &String) -> HashSet<Item> {
    ...
    fn items(&self) -> Vec<HashSet<Item>> {
        ...
        'repeat_until_no_new_items: loop {
            ...
            for item in &items {
                for symbol in &self.grammar.symbol_set {
                    let goto_set = self.goto(item, symbol);
    ...
    fn action_table(&self) -> ActionTable {
        ...
        for (state_id, item_set) in self.item_sets.clone().into_iter().enumerate() {
            for item in &item_set {
                // If [A -> ɑ·aꞵ, b] is in item_set_i...
                if item.dot < item.production.rhs.len() {
                    if self.is_terminal(&item.production.rhs[item.dot]) {
    ...
    fn goto_table(&self) -> GotoTable {
        ...
        for (state_id, item_set) in self.item_sets.clone().into_iter().enumerate() {
            ...
            for symbol in &self.grammar.symbol_set {
                if !self.is_terminal(symbol) {
                    let goto_item_set = self.goto(&item_set, &symbol);
    ...
    pub fn tables(&self) -> (ActionTable, GotoTable) {
        (self.action_table(), self.goto_table())
    }
