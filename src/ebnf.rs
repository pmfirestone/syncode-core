...
mod tests {
    ...
    #[test]
    fn sugar_and_unsugared_json() {
        let ebnf_grammar_text = &fs::read_to_string("grammars/json_sugar.lark").unwrap();
        let bnf_grammar_text = &fs::read_to_string("grammars/json.lark").unwrap();

        let ebnf_grammar = EBNFParser::new(&ebnf_grammar_text, "json").parse();
        let bnf_grammar = EBNFParser::new(&bnf_grammar_text, "json").parse();

        println!("ebnf: {:#?}", ebnf_grammar);
        println!("bnf: {:#?}", bnf_grammar);
    }

    #[bench]
    fn parse_json_grammar(b: &mut test::Bencher) {
        let grammar_text = &fs::read_to_string("grammars/json.lark").unwrap();
        b.iter(|| EBNFParser::new(&grammar_text, "json").parse());
    }
}
